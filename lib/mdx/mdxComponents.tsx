...
const LinkComponent = (props: PropsWithChildren<HTMLAnchorElement>) => (
  <a
    ...
    className={cn(
      ...
      props.target === "_blank" && "mr-4",
    )}
  >
    {props.children}{" "}
    {props.target === "_blank" && (
      <ArrowUpRight className="absolute -right-3.5 w-3 top-0" />
    )}
  </a>
);
...
export default function mdxComponents(type?: "blog" | "lp" | undefined) {
  const components = {
    ...
    Subtitle: (props: any) => (
      <p
        className={cn("-mt-6 text-foreground-lighter text-lg", props.className)}
        {...props}
      />
    ),
    Admonition,
