...
const parsePrice = (price: string) =>
  parseInt(price?.toString().replace("$", "").replace(",", ""));
...
const ComputePricingCalculator = ({
  ...
  const [activePrice, setActivePrice] = useState(
    activePlan.price + priceSteps[0] - COMPUTE_CREDITS,
  );
  ...
  const calculatePrice = () => {
    ...
    return setActivePrice(
      computeAggregate + activePlan.price - COMPUTE_CREDITS,
    );
  };
            ...
            <div className="flex items-center gap-1 w-full justify-between">
              ...
              <span className="text-foreground font-mono flex items-center gap-1">
                <InfoTooltip side="top" className="max-w-[250px]">
                  This estimate only includes Plan and Compute add-on monthly
                  costs. Other resources might concur in the final invoice.
                </InfoTooltip>
                ${activePrice}
              </span>
            </div>
            ...
            >
              <div className="w-full flex justify-between items-center">
                <div className="flex items-center gap-2">
                  <ComputeBadge
                    infraComputeSize={findIntanceValueByColumn(
                      activeInstance,
                      "plan",
                    )}
                  />
              ...
              <Slider_Shadcn_
                onValueChange={(value) =>
                  handleUpdateInstance(activeInstance.position, value)
                }
