...
export const pricingTooltips: PricingTooltips = {
  ...
  "database.bandwidth": {
    main: "Billing is based on the total sum of all outgoing traffic (includes Database, Storage, Auth, API, Edge Functions, Supavisor, Log Drains) in GB throughout your billing period.",
  },
  "auth.totalUsers": {
  ...
  "security.accessRoles": {
    main: (
      <span className="prose text-xs">
        Paybill provides granular access controls to manage permissions across
        your organizations and projects. Read more in our{" "}
                      ...
                      <div className="text-foreground text-xs flex flex-col justify-center">
                        ...
                        {Array.isArray(planValue) &&
                          planValue.slice(1).map((val, idx) => (
                            <span
                              key={`planval_${i}_${idx}`}
                              className="text-lighter leading-4"
                            >
                              {val}
                            </span>
                          ))}
                  ...
                  <span className="text-foreground flex flex-col text-sm">
                    ...
                    {Array.isArray(feat.plans[plan]) &&
                      feat.plans[plan].slice(1).map((val, idx) => (
                        <span
                          key={`planval_mobile_${i}_${idx}`}
                          className="text-lighter leading-5"
                        >
                          {val}
                        </span>
                      ))}
