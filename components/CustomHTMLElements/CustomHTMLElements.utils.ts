  ...
  if (typeof text === "object") {
    if (Array.isArray(text)) {
      const customAnchor = text.find(
        (x) => typeof x === "string" && hasCustomAnchor(x),
      );
...
const hasCustomAnchor = (value: string): boolean =>
...
export const highlightSelectedTocItem = (id: string) => {
  ...
  const currentActiveItem = document.querySelector(
    ".toc-menu .toc__menu-item--active",
  );
...
export const highlightSelectedNavItem = (id: string) => {
  const navMenuItems = document.querySelectorAll<HTMLAnchorElement>(
    ".function-link-item a",
  );

  // find any currently active items and remove them
  const currentActiveItems = document.querySelectorAll(
    ".function-link-list .text-brand",
  );
